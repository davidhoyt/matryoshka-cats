...
trait Recursive[T] extends Based[T] { self =>
  ...
  @Deviation("Uses Eval.")
  def isLeaf(t: T)(implicit BF: Functor[Base], B: Foldable[Base]): Eval[Boolean] =
    project(t).foldRight(Eval.True)((e, a) => Eval.False)
  ...
  def children[U]
    ...
    (implicit U: Corecursive.Aux[U, ListF[T, ?]], BF: Functor[Base], B: Foldable[Base])
      : Eval[U] =
    project(t).foldRight[U](Eval.now(NilF[T, U]().embed))((t, eu) => eu.map(u => ConsF(t, u).embed))
  ...
  @Deviation("Uses and returns Eval.")
  def collect[U: Monoid, B]
    (t: T)
    (pf: PartialFunction[T, B])
    (implicit U: Corecursive.Aux[U, ListF[B, ?]], BF: Functor[Base], B: Foldable[Base])
      : Eval[U] =
    foldMap(t)(pf.lift(_).foldRight[U](Eval.now(NilF[B, U]().embed))((b, eu) => eu.map(u => ConsF(b, u).embed)))
  ...
  @Deviation("Refers to Trampoline instead of Free.Trampoline.")
  def foldMap[Z: Monoid]
    (t: T)
    (f: T => Z)
    (implicit BF: Functor[Base], B: Foldable[Base])
      : Z =
    foldMapM[Trampoline, Z](t)(f(_).pure[Trampoline]).run
...
object Recursive {
  def fromCoalgebra[T, F[_]](ψ: Coalgebra[F, T]): Aux[T, F] = new Recursive[T] {
    type Base[A] = F[A]
    def project(t: T)(implicit BF: Functor[Base]): F[T] = ψ(t)
  }
  ...
  trait Ops[T, F[_]] {
    ...
    def isLeaf(implicit BT: Traverse[F]): Eval[Boolean] =
      typeClassInstance.isLeaf(self)
    def children[U](implicit U: Corecursive.Aux[U, ListF[T, ?]], BT: Traverse[F]): Eval[U] =
