...
package object matryoshka {
  ...
  implicit def GAlgebraZip[W[_]: Functor, F[_]: Functor]:
    ...
    new Zip[GAlgebra[W, F, ?]] {
      def zip[A, B](a: Eval[GAlgebra[W, F, A]], b: Eval[GAlgebra[W, F, B]]): GAlgebra[W, F, (A, B)] =
        node => (a.value(node ∘ (_ ∘ (_._1))), b.value(node ∘ (_ ∘ (_._2))))
    }
  /**
    *
    * @group algtrans
  ...
  implicit def ElgotAlgebraMZip[W[_]: Functor, M[_]: Applicative, F[_]: Functor]:
    ...
    new Zip[ElgotAlgebraM[W, M, F, ?]] {
      def zip[A, B](a: Eval[ElgotAlgebraM[W, M, F, A]], b: Eval[ElgotAlgebraM[W, M, F, B]]): ElgotAlgebraM[W, M, F, (A, B)] =
        w => Bitraverse[(?, ?)].bisequence((a.value(w ∘ (_ ∘ (_._1))), b.value(w ∘ (_ ∘ (_._2)))))
    }
  /**
    *
    * @group algtrans
  ...
  def count[T: Equal, F[_]: Functor: Foldable]
    ...
    e => (e._1 ≟ form).fold(1, 0) + e._2.foldRight(Eval.now(0))((i, eb) => eb.map(b => i + b)).value
