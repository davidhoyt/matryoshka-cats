...
package object fixedpoint {
  ...
  implicit def recursiveTListFFoldable[T[_[_]]: RecursiveT]: Foldable[λ[α => T[ListF[α, ?]]]] =
    new Foldable[λ[α => T[ListF[α, ?]]]] {
      ...
      override def foldLeft[A, B](fa: T[ListF[A, ?]], z: B)(f: (B, A) => B): B =
        fa.cata[B] {
          case NilF() => z
          case ConsF(a, b) => f(b, a)
        }

      override def foldRight[A, B](fa: T[ListF[A, ?]], z: Eval[B])(f: (A, Eval[B]) => Eval[B]): Eval[B] =
        fa.cata[Eval[B]] {
          case NilF() => z
          case ConsF(a, b) => f(a, b)
        }
  ...
  implicit def birecursiveListFMonoid[T, A](implicit T: Birecursive.Aux[T, ListF[A, ?]])
      : Monoid[T] =
    new Monoid[T] {
      def empty: T = NilF[A, T]().embed
      def combine(f1: T, f2: T): T = f1.cata[T] {
        case NilF() => f2
  ...
  implicit def recursiveTEitherFoldable[T[_[_]]: RecursiveT]
    ...
    new Foldable[λ[α => T[α \/ ?]]] {
      override def foldMap[A, B: Monoid](fa: T[A \/ ?])(f: A ⇒ B): B =
        fa.cata[B](_.leftMap(f).merge)

      override def foldLeft[A, B](fa: T[A \/ ?], z: B)(f: (B, A) => B): B =
        fa.cata[B](_.leftMap(f(z, _)).merge)

      override def foldRight[A, B](fa: T[A \/ ?], z: Eval[B])(f: (A, Eval[B]) => Eval[B]): Eval[B] =
  ...
  implicit val partialMonad: Monad[Partial] = new Monad[Partial] {
    ...
    override def tailRecM[Z, B](z: Z)(f: Z => Partial[Either[Z, B]]): Partial[B] = {
      ...
      val faa: Either[Either[Z, B], B] = fa.a
      faa match {
        case Right(b) => Partial.now(b)
        case Left(Right(b)) => Partial.now(b)
        case Left(Left(a)) => tailRecM(a)(f)
      }
    }

    override def flatMap[A, B](fa: Partial[A])(f: A => Partial[B]): Partial[B] =
  ...
  implicit class PartialOps[A](self: Partial[A]) {
    ...
    def runFor[N](steps: N)(implicit N: Recursive.Aux[N, Option]): A \/ Partial[A] =
      (steps, self).anaM[Partial[A]] {
        case (r, p) =>
          r.project.fold[(A \/ Partial[A]) \/ (A \/ (N, Partial[A]))](
            p.project.left.left)(
